...
const MobileNavItem = (props: {
  ...
  if (props.href) {
    return (
      <li>
        <a
          className="flex items-center justify-between px-4 py-3 font-semibold text-md text-primary hover:bg-primary_hover"
          href={props.href}
        >
          {props.label}
        </a>
...
const MobileFooter = () => {
  return (
    <div className="flex flex-col gap-8 border-secondary border-t px-4 py-6">
      <div>
        <ul className="grid grid-flow-col grid-cols-2 grid-rows-4 gap-x-6 gap-y-3">
          {footerNavItems.map((navItem) => (
            <li key={navItem.label}>
              <Button color="link-gray" href={navItem.href} size="lg">
                {navItem.label}
              </Button>
            </li>
          ))}
              ...
              <ul className="flex items-center gap-0.5">
                {items.map((navItem) => (
                  <li key={navItem.label}>
                    {navItem.menu ? (
                      <AriaDialogTrigger>
                        <AriaButton className="flex cursor-pointer items-center gap-0.5 rounded-lg px-1.5 py-1 font-semibold text-md text-secondary outline-focus-ring transition duration-100 ease-linear hover:text-secondary_hover focus-visible:outline-2 focus-visible:outline-offset-2">
                          <span className="px-0.5">{navItem.label}</span>

                          <ChevronDown className="size-4 in-aria-expanded:-rotate-180 rotate-0 stroke-[2.625px] text-fg-quaternary transition duration-100 ease-linear" />
                        </AriaButton>
                        ...
                        <AriaPopover
                          className={({ isEntering, isExiting }) =>
                            cx(
                              ...
                              isExiting &&
                                "fade-out slide-out-to-top-1 animate-out duration-150 ease-in"
                            )
                          }
                          containerPadding={0}
                          ...
                          {({ isEntering, isExiting }) => (
                            <AriaDialog
                              className={cx(
                                ...
                                isEntering &&
                                  !isFullWidth &&
                                  "zoom-in-95 animate-in duration-200 ease-out",
                                isExiting &&
                                  !isFullWidth &&
                                  "zoom-out-95 animate-out duration-150 ease-in"
                              )}
                            >
                              {navItem.menu}
                            </AriaDialog>
                      ...
                      <a
                        className="flex cursor-pointer items-center gap-0.5 rounded-lg px-1.5 py-1 font-semibold text-md text-secondary outline-focus-ring transition duration-100 ease-linear hover:text-secondary_hover focus:outline-offset-2 focus-visible:outline-2"
                        href={navItem.href}
                      >
                        <span className="px-0.5">{navItem.label}</span>
                      </a>
